...
def main():
    ...
    for item in filepaths:                      # iterate through the whole list
        with open(item, "r") as f:              # open the files in read-only
            data = f.read().split("\n")         # read the whole file and immediate split it by line
            for i in range(len(data)):              # go through each line in that file
                tProLine = data[i].count("\t")      # count the tabs in that line
                if tProLine >= tMax:                # if it's "deeper" than the previous record
                    tMax = tProLine                 # update maximum depth
...
if __name__ == "__main__":                      # for compatibility with other pyton scripts
    main()
