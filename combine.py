...
def main():
    ...
    for item in filepaths:                                  # go through each file
        ...
        with open(destination, "a") as g:                   # open the destination file
            while data.find("<lido:lido>") != -1:           # as long as there's another entry
                ...
                data = data[posB:]                          # remove that entry from the original data
                remaining = round(len(data) / 2**20, 3)                         # how much data is left in this file in MB
                sys.stdout.write("\r{} MB remaining.".format(remaining))
        print("\nWrote that part.")
